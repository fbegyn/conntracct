...
func (s *DuckDBSink) newBatch() {
	s.batch = make(batch, 0, s.config.BatchSize)
	s.stats.SetBatchLength(0)
}

func (s *DuckDBSink) addBatchEvent(e *event) {
	s.batchMu.Lock()
...
func (s *DuckDBSink) sendBatch(b batch) {
	...
	err := s.appender.Flush()
	if err != nil {
		log.WithField("sink", s.config.Name).Error("failed execute appender operation. Error: %v", err)
	}
}
